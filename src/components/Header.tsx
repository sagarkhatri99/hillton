                ...
                <div className="flex justify-between items-center h-20">
                    ...
                    <nav className="hidden lg:flex items-center gap-8 font-sans text-sm font-medium text-stone-600">
                        ...
                        <div className="relative group">
                            ...
                            <div className="absolute top-full left-0 w-64 bg-white shadow-lg rounded-lg py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-stone-100">
                                <Link href="/conditions" className="block px-4 py-2 hover:bg-cream-100 text-stone-700">All Conditions</Link>
                                <Link href="/conditions/arthritis-joint-pain" className="block px-4 py-2 hover:bg-cream-100 text-stone-700">Arthritis & Pain</Link>
                                <Link href="/conditions/obesity-weight-gain" className="block px-4 py-2 hover:bg-cream-100 text-stone-700">Weight Management</Link>
                                <Link href="/conditions/lifestyle-disorders" className="block px-4 py-2 hover:bg-cream-100 text-stone-700">Lifestyle Disorders</Link>
                    ...
                    <div className="hidden lg:flex items-center gap-4">
                        <a href="tel:+919266423944" className="flex items-center gap-2 text-primary font-medium text-sm">
                            <Phone size={16} /> <span className="hidden xl:inline">+91 9266423944</span>
                        </a>
                        <Link href="/contact" className="btn-hover bg-primary hover:bg-primary-hover text-white px-5 py-2.5 rounded-full text-sm font-medium transition shadow-md hover:shadow-lg">
                            Book Consultation
                        </Link>
                    </div>
            ...
            {isOpen && (
                <div className="lg:hidden bg-white/95 backdrop-blur-md border-t border-stone-100 absolute w-full h-[calc(100vh-80px)] overflow-y-auto mobile-menu-enter">
                    <div className="container mx-auto px-4 py-6 space-y-6">
                        <Link href="/" className="block text-lg font-medium text-stone-800 hover:text-primary transition" onClick={() => setIsOpen(false)}>Home</Link>
                        <Link href="/about" className="block text-lg font-medium text-stone-800 hover:text-primary transition" onClick={() => setIsOpen(false)}>About</Link>
                        ...
                        <div>
                            <button className="flex items-center justify-between w-full text-lg font-medium text-stone-800 mb-2 hover:text-primary transition" onClick={() => setConditionsOpen(!conditionsOpen)}>
                                Conditions <ChevronDown size={16} className={`transform transition-transform duration-300 ${conditionsOpen ? 'rotate-180' : ''}`} />
                            </button>
                            <div className={`pl-4 space-y-3 border-l-2 border-stone-100 ml-1 overflow-hidden transition-[max-height] duration-300 ${conditionsOpen ? 'max-h-96' : 'max-h-0'}`}>
                                <Link href="/conditions" className="block text-stone-600 hover:text-primary" onClick={() => setIsOpen(false)}>All Conditions</Link>
                                <Link href="/conditions/arthritis-joint-pain" className="block text-stone-600 hover:text-primary" onClick={() => setIsOpen(false)}>Arthritis</Link>
                                <Link href="/conditions/obesity-weight-gain" className="block text-stone-600 hover:text-primary" onClick={() => setIsOpen(false)}>Weight Loss</Link>
                                <Link href="/conditions/lifestyle-disorders" className="block text-stone-600 hover:text-primary" onClick={() => setIsOpen(false)}>Lifestyle Disorders</Link>
                            </div>
                        </div>
                        ...
                        <div className="pt-6 border-t border-stone-100">
                            <Link href="/contact" className="btn-hover block w-full text-center bg-primary text-white py-3 rounded-lg font-medium mb-4" onClick={() => setIsOpen(false)}>
                                Book Consultation
                            </Link>
                            <a href="tel:+919266423944" className="flex items-center justify-center gap-2 text-primary font-medium hover:text-primary-hover">
